        ...
        public struct MinimapGeneratingSetup
        {
            ...
            public void GenerateFieldMinimap(FieldSetup fs, FGenGraph<FieldCell, FGenPoint> grid, Transform t)
            {
                ...
                if (GenerateMode == EGenerateMode.EachCellIsPixel)
                {
                    ...
                    foreach (var cell in grid.AllApprovedCells)
                    {
                        // Compute the base pixel position (scaled by the multiplier)
                        Vector2Int basePos = GetPixelPos(gridCellSpaceBounds, cell.Pos, Vector2.one) * multiplier;

                        // For each side, if no neighbor exists, we trim that edge by one pixel.
                        // The cell covers pixels [0, multiplier-1] along each axis.
                        int xMin = grid.CellExists(cell.Pos + Vector3Int.left) ? 0 : 1;
                        int xMax = grid.CellExists(cell.Pos + Vector3Int.right) ? multiplier - 1 : multiplier - 2;
                        int yMin = grid.CellExists(cell.Pos + Vector3Int.back) ? 0 : 1;
                        int yMax = grid.CellExists(cell.Pos + Vector3Int.forward) ? multiplier - 1 : multiplier - 2;

                        // Paint all pixels in the computed rectangle.
                        for (int x = xMin; x <= xMax; x++)
                        {
                            for (int y = yMin; y <= yMax; y++)
                            {
                                PaintPx(basePos + new Vector2Int(x, y), PaintColor, pixels, maxPixel);
                            }
                        }

                        // If the cell is on the border of the grid, paint a border around it.
                        //COrners
                        Color borderColor = Color.black;
                        if (!grid.CellExists(cell.Pos + Vector3Int.left + Vector3Int.back))
                            PaintPx(basePos + new Vector2Int(0, 0), borderColor, pixels, maxPixel);
                        if (!grid.CellExists(cell.Pos + Vector3Int.right + Vector3Int.back))
                            PaintPx(basePos + new Vector2Int(multiplier - 1, 0), borderColor, pixels, maxPixel);
                        if (!grid.CellExists(cell.Pos + Vector3Int.left + Vector3Int.forward))
                            PaintPx(basePos + new Vector2Int(0, multiplier - 1), borderColor, pixels, maxPixel);
                        if (!grid.CellExists(cell.Pos + Vector3Int.right + Vector3Int.forward))
                            PaintPx(basePos + new Vector2Int(multiplier - 1, multiplier - 1), borderColor, pixels, maxPixel);

                        //Borders
                        for (int x = 0;x<xMin; x++)                        
                            for (int y = yMin;y<=yMax;y++)                            
                                PaintPx(basePos + new Vector2Int(x, y), borderColor, pixels, maxPixel);                        

                        for (int x = xMax + 1; x < multiplier; x++)
                            for (int y = yMin; y <= yMax; y++)
                                PaintPx(basePos + new Vector2Int(x, y), borderColor, pixels, maxPixel);
                        ...
                        for (int y = yMax + 1; y < multiplier; y++)
                            for (int x = xMin; x <= xMax; x++)
                                PaintPx(basePos + new Vector2Int(x, y), borderColor, pixels, maxPixel);
