...
namespace FIMSpace.Generating.Planning.PlannerNodes.Field.Special
{
    ...
    public class PR_TightPlacement : PlannerRuleBase
    {
        ...
        public override Vector2 NodeSize { get { return new Vector2( 202, _EditorFoldout ? 216 : 179 ); } }
        ...
        [Tooltip( "Aligning only with Z-Forward rotation (useful for prefab fields)" )]
        [HideInInspector] public bool ForceForwardAlign = false;
        ...
        protected CheckerField3D alignToCheckerEdges;
        ...
        public override void Execute( PlanGenerationPrint print, PlannerResult newResult )
        {
            ...
            for( int a = 0; a < _snapTo.Count; a++ )
            {
                ...
                alignToCheckerEdges = alignToChecker;
        ...
        {
            if( ForceForwardAlign )
            {
                if( checker.FindCellOfInDir( alignToCheckerEdges, checker.RootRotation * Vector3.forward ) == null )
                {
                    skipIteration = true;
                }
            }
